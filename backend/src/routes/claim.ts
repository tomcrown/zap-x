  ...
  async (req: AuthenticatedRequest, res, next) => {
    ...
    `;
      ...
      const authorizedWallet =
        dbUser?.wallet_address || req.user!.walletAddress;

      if (
        authorizedWallet &&
        normalizeAddress(recipientWallet) !== normalizeAddress(authorizedWallet)
      ) {
        res.status(403).json({
          error: "Recipient wallet does not match authenticated user.",
        });
        return;
      }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res, next) => {
    try {
      const result = await cancelClaimLink(
        req.params.token,
