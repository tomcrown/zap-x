...
import { executeLendingDeposit, executeLendingWithdraw } from '../lib/starkzap.js';
...
export function LendingPage() {
  ...
  const selectedTokenInfo = SUPPORTED_TOKENS.find((t) => t.value === depositToken)!;

  const handleDeposit = async () => {
    if (!walletAddress || !depositAmount || parseFloat(depositAmount) <= 0) return;
    setDepositing(true);
    try {
      const txHash = await executeLendingDeposit(depositToken, depositAmount, true);
      await lendingApi.deposit({ token: depositToken, amount: depositAmount, txHash });
  ...
  const handleWithdraw = async (position: LendingPosition) => {
    ...
    try {
      const txHash = await executeLendingWithdraw(position.token as TokenSymbol, position.supplied_amount);
      await lendingApi.withdraw(position.id, txHash);
      toast({ type: 'success', title: 'Withdrawn!', message: 'Funds returned to your wallet.', txHash });
      queryClient.invalidateQueries({ queryKey: ['lending-stats'] });
    } catch (err: any) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-white">Lend & Earn</h1>
          ...
          {SUPPORTED_TOKENS.map((t) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{t.label.split('—')[1]?.trim()}</p>
              </div>
      ...
      <div className="card">
        ...
        {isLoading ? (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`badge ${pos.status === 'active' ? 'badge-active' : 'badge-expired'}`}>
                        {pos.status}
                      </span>
                    </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-slate-600 mt-1">Supply tokens to start earning yield</p>
          </div>
        )}
      </div>

      {/* Deposit Modal */}
      <Modal open={depositModal} onClose={() => setDepositModal(false)} title="Supply Tokens">
        <div className="space-y-4">
          <Select
            label="Token"
            value={depositToken}
            onChange={(e) => setDepositToken(e.target.value as TokenSymbol)}
            options={SUPPORTED_TOKENS.map((t) => ({ value: t.value, label: t.label }))}
          />

          <div className="p-4 rounded-xl bg-surface border border-surface-border">
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-400">Available</span>
              <span className="font-mono font-bold text-white">{available.toFixed(4)} {depositToken}</span>
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Amount to Supply</label>
            ...
            <div className="p-3 rounded-xl bg-surface border border-surface-border text-sm space-y-1">
              <div className="flex justify-between text-slate-400">
                ...
                <span className="text-green-400 font-mono">
                  ~{(parseFloat(depositAmount) * 0.06).toFixed(4)} {depositToken}
                </span>
              </div>
              <div className="flex justify-between text-slate-400">
