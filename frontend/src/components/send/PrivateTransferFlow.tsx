...
export function PrivateTransferFlow({
  ...
  const stepOrder: PrivateTransferStep[] = [
    "initializing",
    "funding",
    "transferring",
  ];
      ...
      <div className="space-y-2">
        {STEPS.map((step) => {
          ...
          return (
            <div
              key={step.key}
              className={`flex items-start gap-3 transition-opacity ${isSkipped ? "opacity-30" : "opacity-100"}`}
            >
              <StepIcon state={state} />
              <div className="flex-1 min-w-0 pt-1">
                <p
                  className={`text-sm font-medium leading-tight ${state === "active" ? "text-white" : state === "done" ? "text-zinc-300" : "text-zinc-600"}`}
                >
                  {step.label}
