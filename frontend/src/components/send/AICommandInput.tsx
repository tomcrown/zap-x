...
export function AICommandInput({ onActions }: Props) {
  ...
  const examples = [
    'Send 5 STRK to @alice',
    'Send 10 STRK to bob@example.com and stake 2 STRK',
    'Stake 50 STRK',
    ...
    <div className="card border-brand-500/30 bg-gradient-to-br from-surface-card to-brand-950/20">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Powered by Gemini — type naturally</p>
        </div>
      </div>
        ...
        <div className="mt-4 animate-fade-in">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-slate-400 font-medium">
              Parsed {result.actions.length} action{result.actions.length !== 1 ? 's' : ''}
          ...
          {result.clarification && (
            <p className="text-xs text-yellow-400 mb-3">⚠ {result.clarification}</p>
          )}

          <Button onClick={handleExecuteAll} className="w-full">
            Execute {result.actions.length} Action{result.actions.length !== 1 ? 's' : ''}
          </Button>
        </div>
      )}
      ...
      {result && result.actions.length === 0 && (
        <div className="mt-3 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-sm text-yellow-400">
          {result.clarification ?? 'Could not extract any actions from that command. Try rephrasing.'}
        </div>
      )}
    </div>
  );
...
function ActionPreview({ action, index }: { action: ParsedAction; index: number }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg bg-surface border border-surface-border">
      ...
      <div className="flex-1 min-w-0">
        <span className="text-xs text-slate-500 uppercase font-semibold">Action {index}</span>
        ...
        {action.note && <p className="text-xs text-slate-500 mt-0.5 italic">"{action.note}"</p>}
      </div>
    </div>
  );
}
