    ...
    <div className="w-full">
      ...
      <div className="relative">
        <input
          ...
          className={`input pr-10 ${
            resolved ? "border-green-500" : notFound ? "border-red-500/60" : ""
          }`}
        />
      ...
      {resolved && (
        <div className="mt-1.5 flex items-center gap-2 text-xs text-green-400">
          ...
          {resolved.username && (
            <span className="text-slate-500">(@{resolved.username})</span>
          )}
        </div>
      )}
      ...
      {isEmail && !resolving && !resolved && value.length > 5 && (
        <p className="mt-1.5 text-xs text-yellow-400">
          📧 Email not registered — a claim link emailed to them. recipients
          should check spam folder.
        </p>
      )}
