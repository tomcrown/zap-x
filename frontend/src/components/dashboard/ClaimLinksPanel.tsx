...
function ClaimRow({
  ...
  const statusDisplay =
    isExpired && claim.status === "pending" ? "expired" : claim.status;
  ...
  const recipient =
    claim.recipientEmail ?? claim.recipientUsername ?? "Unknown";
  const expiryLabel = new Date(claim.expiresAt).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
  });
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-surface-border last:border-0">
      ...
      <div className="text-right shrink-0 ml-4 space-y-1">
        ...
        <span
          className={`badge ${statusClass[statusDisplay] ?? "badge-expired"}`}
        >
          {statusDisplay}
        </span>
            ...
            <button
              onClick={() =>
                navigator.clipboard.writeText(
                  `${window.location.origin}/claim/${claim.token}`,
                )
              }
      ...
      {claims.length === 0 ? (
        <div className="text-center py-10">
          ...
          <p className="text-slate-600 text-xs mt-1">
            Send to an email to generate a claim link
          </p>
        </div>
      ) : (
